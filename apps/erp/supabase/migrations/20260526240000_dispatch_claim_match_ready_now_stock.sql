-- La liste « prêts à contacter » et les KPIs sont basés sur dispatch_queue_status = ready_now.
-- La RPC d’attribution exigeait encore stock_status = ready + qualification = qualified, ce qui
-- excluait tout le stock importé en new / to_validate — d’où « Aucune fiche disponible » malgré 587+ en file.
...
AS $$
  ...
  picked AS (
    ...
    WHERE v.n < 120
      AND lgs.stock_status IN ('new', 'ready')
      AND lgs.qualification_status IN ('to_validate', 'qualified', 'pending')
      AND lgs.phone_status = 'found'
      AND lgs.converted_lead_id IS NULL
    ...
    LIMIT LEAST(
      ...
      GREATEST(0, 120 - (SELECT n FROM agent_volume))
    )
  ),
  ins AS (
...
COMMENT ON FUNCTION public.dispatch_lead_generation_stock_claim(
  ...
  'Sélection ready_now : stock new|ready, qualif to_validate|qualified|pending (aligné prêts à contacter) ; limite = min(p_limit, 120 − vol. agent).';
