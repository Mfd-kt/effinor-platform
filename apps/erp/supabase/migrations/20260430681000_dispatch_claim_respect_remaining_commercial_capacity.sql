-- Plafond strict : une attribution ne peut pas dépasser la capacité résiduelle (120 − volume opérationnel agent).
-- Évite p.ex. 20 fiches attribuées quand il ne reste que 8 places avant 120.
...
COMMENT ON FUNCTION public.dispatch_lead_generation_stock_claim(
  ...
  'Sélection ready_now (SKIP LOCKED) ; limite = min(p_limit, 120 − volume opérationnel agent).';
...
COMMENT ON FUNCTION public.dispatch_lead_generation_stock_claim_premium(uuid, integer, integer) IS
  'Attribution SKIP LOCKED — premium ; limite = min(p_limit, 120 − volume opérationnel agent).';
