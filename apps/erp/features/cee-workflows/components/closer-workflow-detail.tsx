...
export function CloserWorkflowDetail({
  ...
  detail: CloserWorkflowDetailData;
  ...
  /** La simulation agent vit souvent dans le workflow, pas dans `leads.sim_*` : on fusionne pour l’affichage. */
  const leadForFullPanel = useMemo(() => {
    if (!detail.leadDetail) return null;
    return mergeLeadDetailWithWorkflowSimulationResult(
      detail.leadDetail,
  ...
  return (
    <div className="space-y-5">
      <Card className="border-border/80 shadow-sm">
        ...
        <CardContent className="grid gap-4 md:grid-cols-2 text-sm">
          ...
          <div className="space-y-2">
            ...
            <div><strong>Closer :</strong> {workflow.assigned_closer?.full_name || workflow.assigned_closer?.email || "Non affecté"}</div>
          </div>
        </CardContent>
      </Card>

      {workflow.lead?.id ? (
        <CloserLeadFullDetailPanel
          lead={leadForFullPanel}
          leadId={workflow.lead.id}
          leadSourceForEdit={detail.leadDetail}
      ...
      <AgentSimulationResultCard
        result={
          workflow.simulation_result_json &&
          typeof workflow.simulation_result_json === "object" &&
          !Array.isArray(workflow.simulation_result_json)
            ? (workflow.simulation_result_json as never)
            : null
        }
      ...
      <Card className="border-border/80 shadow-sm">
        <CardHeader>
          <CardTitle>Notes transmises</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3 text-sm">
          <div>
            <div className="font-medium">Notes agent</div>
