...
const TAB_IDS = new Set<QueueTab>(["pending", "qualified", "docsReady", "recent"]);

const TABS: { id: QueueTab; label: string; count: (q: ConfirmateurQueueBuckets) => number }[] = [
  { id: "pending", label: "À confirmer", count: (q) => q.pending.length },
  { id: "qualified", label: "Qualifiés", count: (q) => q.qualified.length },
  { id: "docsReady", label: "Docs prêts", count: (q) => q.docsReady.length },
  { id: "recent", label: "Récents", count: (q) => q.recent.length },
];

function itemsForTab(queue: ConfirmateurQueueBuckets, tab: QueueTab): ConfirmateurQueueItem[] {
  switch (tab) {
    case "pending":
    ...
    case "docsReady":
      return queue.docsReady;
    default:
      return queue.recent;
  }
}

function filterItems(items: ConfirmateurQueueItem[], q: string): ConfirmateurQueueItem[] {
...
function sortItems(items: ConfirmateurQueueItem[], key: SortKey, dir: "asc" | "desc"): ConfirmateurQueueItem[] {
...
export function ConfirmateurWorkflowQueue({
  queue,
  onOpen,
  initialQueueTab,
}: {
  queue: ConfirmateurQueueBuckets;
  onOpen: (item: ConfirmateurQueueItem) => void;
  ...
  return (
    <Card className="overflow-hidden border-border/80 shadow-sm">
      <CardHeader className="border-b bg-muted/20 pb-4">
        ...
        <CardDescription>
          Vues par étape du pipeline — ouvrez une ligne pour la page de confirmation (style tableau, comme un CRM).
        </CardDescription>
        <div className="mt-4 flex flex-wrap gap-2" role="tablist" aria-label="Étapes du pipeline">
          {TABS.map((t) => (
            <CrmQueueTabButton
              key={t.id}
              label={t.label}
          ...
          <Table className="min-w-[920px]">
            <TableHeader className="sticky top-0 z-10 shadow-[0_1px_0_hsl(var(--border))]">
              <TableRow className="border-0 hover:bg-transparent">
                ...
                />
                <TableHead className="min-w-[180px] bg-muted/80 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  Contact
                </TableHead>
                <CrmSortableHead
                  label="Économie / an"
              ...
              {rows.length === 0 ? (
                ...
                rows.map((item) => (
                  <TableRow
                    ...
                    onClick={() => onOpen(item)}
                  >
                    <TableCell className="max-w-[220px] pl-4 font-medium" title={item.companyName}>
