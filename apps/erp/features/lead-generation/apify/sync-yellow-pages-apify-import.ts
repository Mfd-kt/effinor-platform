...
import { getApifyDatasetItems, getApifyRun, getApifyToken, isApifyRunFinished } from "./client";
import { mapYellowPagesApifyItem } from "./map-yellow-pages-apify-item";
...
/** Même machine d’état que Google Maps, mapper et source différents. */
export async function syncYellowPagesApifyImport(input: {
  batchId: string;
  ...
  if (row.source !== "apify_yellow_pages") {
    return {
      phase: "invalid_batch",
      batchId,
      message: "Ce batch n’est pas un import apify_yellow_pages.",
    };
  }
  ...
  for (let i = 0; i < rawItems.length; i++) {
    const mapped = mapYellowPagesApifyItem(rawItems[i], i);
  ...
  if (deferIngest) {
    ...
    return {
      ...
      duplicateCount: 0,
      rejectedCount: 0,
      message: "Dataset Pages Jaunes récupéré — fusion multi-source côté coordinateur.",
    };
  }
