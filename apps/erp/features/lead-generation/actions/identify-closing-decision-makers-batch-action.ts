...
import { identifyClosingLeadGenerationDecisionMakersBatchActionInputSchema } from "../schemas/lead-generation-actions.schema";
import {
...
export async function identifyClosingLeadGenerationDecisionMakersBatchAction(
  input: unknown,
  ...
  const parsed = identifyClosingLeadGenerationDecisionMakersBatchActionInputSchema.safeParse(input ?? {});
  if (!parsed.success) {
  ...
  try {
    ...
    const raw = e instanceof Error ? e.message : "Erreur lors de l’identification des décideurs (closing).";
