...
import { startPapImportAction } from "@/features/lead-generation/actions/start-pap-import-action";

const PAP_DEFAULT_MAX = 1000;
const PAP_MAX = 1000;

/** URL large (scraping max) ; le ciblage CEE est appliqué dans `map-item.ts`. */
const PAP_DEFAULT_START_URL = "https://www.pap.fr/annonce/vente-maison";
...
const formSchema = z.object({
  ...
  maxItems: z.coerce.number().int().min(1).max(PAP_MAX).default(PAP_DEFAULT_MAX),
});

type FormInput = z.input<typeof formSchema>;
type FormOutput = z.output<typeof formSchema>;

export function StartPapImportModal() {
  ...
  const form = useForm<FormInput, unknown, FormOutput>({
    ...
    defaultValues: {
      startUrl: PAP_DEFAULT_START_URL,
      maxItems: PAP_DEFAULT_MAX,
    },
  });

  function onSubmit(values: FormOutput) {
    startTransition(async () => {
      const result = await startPapImportAction({
        startUrl: values.startUrl,
        maxItemsToScrape: values.maxItems,
      });

      if (result.ok) {
        toast.success("Import PAP lancé !", {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>Nouvel import — PAP.fr</DialogTitle>
          <DialogDescription>
            Scrape des annonces PAP.fr <strong>avec téléphones des particuliers</strong>. Coût
            indicatif : ~$2 / 1000 annonces.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <div className="flex gap-3 rounded-md border bg-muted/40 p-3 text-sm">
            ...
            <div className="space-y-1 text-xs leading-relaxed text-muted-foreground">
              <p>
                ...
                , appliquez vos filtres CEE (vente · maison · DPE D-G · ville), puis copiez l&apos;URL
                de la page de résultats.
              </p>
              <p>
                Exemple :{" "}
                <code className="rounded bg-background px-1 py-0.5 text-[11px]">
                  https://www.pap.fr/annonce/vente-maison-bordeaux-33000-classes-energie-d-e-f-g
                </code>
              </p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              id="startUrl"
              type="url"
              placeholder="https://www.pap.fr/annonce/vente-maison-..."
              autoComplete="off"
            ...
            <button
              type="button"
              onClick={() => form.setValue("startUrl", PAP_DEFAULT_START_URL)}
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Défaut : {PAP_DEFAULT_MAX}. Plafond : {PAP_MAX} (≈ ${(PAP_MAX * 0.002).toFixed(0)}{" "}
              max).
            </p>
