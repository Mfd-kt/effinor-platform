...
import { syncPapImportAction } from "@/features/lead-generation/actions/sync-pap-import-action";
...
/**
 * Bouton « Synchroniser » : run en cours, ou lot terminé (rattachement / métadonnées rejet).
 ...
 */
export function SyncPapImportButton({ batchId, size = "sm", variant = "outline" }: Props) {
  const [isPending, startTransition] = useTransition();

  function handleClick() {
    startTransition(async () => {
      const result = await syncPapImportAction({ batchId });
      ...
      if (result.insertedCount === 0 && result.duplicateCount === 0 && result.totalFetched === 0) {
        toast.warning("Import terminé — 0 annonce récupérée", {
          description: "Le scraping n'a retourné aucun résultat. Vérifiez l'URL de recherche PAP.",
        });
        return;
      }

      if (result.insertedCount === 0 && result.duplicateCount === 0 && result.totalFetched > 0) {
        toast.success("Rattachement du lot mis à jour", {
          description: `${result.totalFetched} fiche(s) pointée(s) vers ce lot (doublons ou correctif). Rechargez la page détail si besoin.`,
        });
        return;
      }
