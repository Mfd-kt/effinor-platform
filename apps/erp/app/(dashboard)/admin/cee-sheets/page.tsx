...
import { getTechnicalVisitTemplateOptionsForAdmin } from "@/features/technical-visits/template-builder/queries/get-technical-visit-template-options-admin";
...
export default async function AdminCeeSheetsPage() {
  ...
  const [sheets, profiles, networkOverview, technicalVisitTemplateOptions] = await Promise.all([
    getAdminCeeSheets(),
    listAssignableProfiles(),
    getAdminCeeNetworkOverview(),
    getTechnicalVisitTemplateOptionsForAdmin(supabase),
  ]);
  ...
  return (
    <CeeSheetAdminShell
      ...
      technicalVisitTemplateOptions={technicalVisitTemplateOptions}
    />
  );
}
