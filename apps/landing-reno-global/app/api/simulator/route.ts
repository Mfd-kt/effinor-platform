...
export async function POST(request: Request) {
  ...
  const companyName = contactName || 'Prospect landing rénovation globale'

  const simPayload = {
    source: 'website_simulator',
    origin_label: 'landing renovation.effinor.fr',
  ...
  const { data: row, error } = await supabase
    .from('leads')
    .insert({
      // Valeur d'enum dédiée landing Rénovation globale (libellé ERP :
      // « Landing Rénovation globale ») — ajoutée via migration
      // 20260426180000_lead_source_landing_reno_global.sql.
      // sim_version='website-simulator-v1' conservé → l'ERP reconnaît le
      // payload via extractWebsiteSimulatorPayload().
      source: 'landing_reno_global',
      lead_status: 'new',
