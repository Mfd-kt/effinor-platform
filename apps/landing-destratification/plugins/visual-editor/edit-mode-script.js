...
function handleGlobalEvent(event) {
	...
	// Never hijack native form interactions while edit mode is enabled.
	// Without this guard, clicks/focus on inputs can be interrupted.
	if (
		event.target instanceof Element &&
		event.target.closest(
			'input, textarea, select, button, label, [contenteditable="true"], [role="textbox"]'
		)
	) {
		return;
	}
